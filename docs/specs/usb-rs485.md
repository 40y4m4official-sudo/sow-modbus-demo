...
通信解析モードでは、接続済みの USB-RS485 アダプタに対して通信設定の自動推定を実行できる。

- 自動推定は受信専用で行い、現場ラインへ応答を返さない
- 推定中は候補ごとに USB シリアル設定を切り替えるため、接続 / 切断が繰り返される
- 推定候補は `baud rate` / `parity` / `stop bits` の組み合わせで構成する
- `stop bits = 1` を優先し、それで十分な受信評価が得られない場合のみ `stop bits = 2` を試行する
- 候補評価は、受信した Modbus RTU フレームの妥当性をもとに行う
  - CRC が通るフレーム数
  - request / response / exception として解釈できるフレーム数
  - 妥当な slave address / function code の比率
  - CRC error / truncated frame の少なさ
- 推定完了後は最良候補を通信解析設定へ反映し、その条件で再接続する
- 手動で通信設定を変更した場合や USB を切断した場合は、自動推定を中止する